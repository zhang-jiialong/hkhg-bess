...
class CustomWait(wait_base):
    def __call__(self, retry_state: RetryCallState):
        ...
        if isinstance(exception, RateLimitError):
            # RateLimitError 使用更长的指数退避，降低再次触发限流的概率
...
async def dash_openai_complete_if_cache(
    ...
    **kwargs,
    ...
    # Use a valid DashScope model name; "qwen3-8b" is not a chat-completions id.
    ...
    # Prefer explicit key; fall back to env
    key = api_key or os.getenv("DASHSCOPE_API_KEY")
    if not key:
        raise RuntimeError("DASHSCOPE_API_KEY missing")


    # IMPORTANT: send DashScope key directly; do NOT rely on OPENAI_API_KEY here
    openai_async_client = AsyncOpenAI(api_key=key, base_url=DASHSCOPE_BASE_URL_INTL)

    # Clean kwargs we don't want to forward
    kwargs.pop("hashing_kv", None)
    kwargs.pop("keyword_extraction", None)
