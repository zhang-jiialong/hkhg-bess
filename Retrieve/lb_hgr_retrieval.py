...
def make_openai_client(config: dict[str, Any]) -> tuple[OpenAI, str]:
    ...
    return client, emb_cfg.get("model_path") or emb_cfg.get("model")


def make_openai_client_from_values(
    *,
    base_url: str = "",
    api_key: str = "",
    embedding_model: str = "",
...
def cosine_scores(query_vec: np.ndarray, matrix: np.ndarray) -> np.ndarray:
    ...
    return np.dot(mat, query) / denom


def get_hyperedge_level(graph: nx.Graph, hyperedge_name: str) -> int:
    if hyperedge_name not in graph.nodes:
        raise KeyError(f"Graph does not contain hyperedge node: {hyperedge_name}")
...
def load_hyperedge_vectors(vdb_path: str | Path) -> dict[str, np.ndarray]:
    ...
    for idx, item in enumerate(storage["data"]):
        result[item["hyperedge_name"]] = matrix[idx]
    return result
...
def build_answer_prompt(question: str, context: str, answer_style: str = "short") -> str:
    if answer_style == "long":
        answer_instruction = """- The final answer should be factual and fully supported by the retrieved context.
- Answer all parts of the question with enough detail when the question asks for an explanation.
- Avoid unsupported speculation or unrelated background explanation."""
    else:
        answer_instruction = """- The final answer should be concise and factual.
- Prefer a single short phrase, entity name, date, number, or term when that answers the question.
- Avoid redundant wording, speculation, and background explanation."""
    return f"""-Goal-
You are a helpful assistant for question answering over retrieved knowledge-graph evidence.
Given a question and retrieved context, generate the final answer using only the context.
...
def ask_llm(client: OpenAI, model: str, prompt: str) -> dict[str, str]:
    ...
    match = re.search(r"<answer>(.*?)</answer>", generation, flags=re.DOTALL | re.IGNORECASE)
    answer = match.group(1).strip() if match else generation.strip()
    return {"answer": answer, "raw_generation": generation}


def answer_with_retrieval_context(
    *,
    client: OpenAI,
    llm_model: str,
    question: str,
    ...
    prompt = build_answer_prompt(question, retrieval_result.get("context", ""), answer_style=answer_style)
    return ask_llm(client, llm_model, prompt)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Standalone LB-HGR retrieval and QA demo.")
    parser.add_argument("--workdir", required=True, help="Graph work directory containing graphml, vdb_hyperedges and chunks.")
    parser.add_argument("--config", default="", help="Optional runtime_config.json with OpenAI-compatible llm/embedding settings.")
    parser.add_argument("--base-url", default="", help="OpenAI-compatible API base URL. Falls back to OPENAI_BASE_URL.")
    parser.add_argument("--api-key", default="", help="OpenAI-compatible API key. Falls back to OPENAI_API_KEY.")
    parser.add_argument("--embedding-model", default="", help="Embedding model. Falls back to config, OPENAI_EMBEDDING_MODEL, or text-embedding-3-large.")
    parser.add_argument("--llm-model", default="", help="Chat model for final answer generation. Falls back to config, OPENAI_MODEL, or gpt-4o-mini.")
    parser.add_argument("--query", required=True, help="Question/query text.")
    parser.add_argument("--output", default="", help="Optional JSON output path.")
    parser.add_argument("--retrieval-only", action="store_true", help="Only return retrieval result; skip final answer generation.")
    parser.add_argument("--answer-style", choices=["short", "long"], default="short")
    parser.add_argument("--max-level", type=int, default=4)
    parser.add_argument("--top-k-anchor", type=int, default=5)
    parser.add_argument("--top-k-l4-expand", type=int, default=3)
    parser.add_argument("--top-k-l0-expand", type=int, default=5)
...
def resolve_client_and_models(args: argparse.Namespace) -> tuple[OpenAI, str, str]:
    config: dict[str, Any] = {}
    if args.config:
    ...
    return client, embedding_model, llm_model
...
def load_retrieval_inputs(
    ...
    client: OpenAI,
    embedding_model: str,
) -> tuple[nx.Graph, dict[str, Any], dict[str, np.ndarray], OpenAI, str]:
    workdir = Path(workdir)
    graph = load_graph(workdir / "graph_chunk_entity_relation.graphml")
    hyperedge_vector_map = load_hyperedge_vectors(workdir / "vdb_hyperedges.json")
    text_chunks = load_json(workdir / "kv_store_text_chunks.json") or {}
    return graph, text_chunks, hyperedge_vector_map, client, embedding_model


def run_cli() -> None:
    args = parse_args()
    client, embedding_model, llm_model = resolve_client_and_models(args)
    ...
    common = dict(
        ...
        top_k_l4_expand=args.top_k_l4_expand,
        top_k_l0_expand=args.top_k_l0_expand,
        top_k_final=args.top_k_final,
        max_iter=args.max_iter,
        max_level=args.max_level,
    )
    if args.constrained:
        result = multi_order_iterative_retrieval_constrained(
            **common,
        ...
        result = multi_order_iterative_retrieval(**common)
